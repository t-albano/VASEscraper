...
new_vase <- new_vase %>%
  select("School.y", "District", "Enrolled", "Year", "Art_Region",
         "Dimension", "Division", "Gold_Seal") %>%
  rename("School" = School.y) %>%
  na.omit()


write.csv(new_vase, "VASE_cleandata.csv", row.names = FALSE)
```
...
vase1 <- fread("VASE_cleandata.csv")
...
write.csv(vase1, "VASE_cleandata.csv", row.names = FALSE)
